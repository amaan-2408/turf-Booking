  ...
  return (
    <>
          ...
          <div className='container my-5'>
            <div className="row">
              <div className="col-md-8">
                <div>
                  <h3>{turf.name}</h3>
                  <p><i className="fa fa-map-marker" aria-hidden="true"></i>&nbsp;{turf.address}</p>
                  <p>{turf.detail}</p>
                  <h4 className='h4 mt-4'>Gallery</h4>
                  <img src={"http://localhost:3000/turf_images/" + turf.image}
              ...
              <div className="col-md-4">
                ...
                <div className="card my-5">
                  ...
                  <div className="card-body">
                    <DatePicker showIcon dateFormat="dd/MMM/YYYY" minDate={new Date()} selected={startDate} onChange={(date) => { setStartDate(date); handleSlot(date) }} />
                  </div>
                </div>
                <div className="card my-5">
                  <div className="card-header">Booking Time ({turf.time_open}-{turf.time_close})</div>
                    ...
                    <ul className="ks-cboxtags">
                      {
                        timingLoopArr.map((item, index) => {
                          return (
                            <li key={index}>
                              {
                                timingOpen + index <= currTime && startDate.getDate() === new Date().getDate() && startDate.getMonth() === new Date().getMonth() && startDate.getFullYear() === new Date().getFullYear()
                                  ?
                                  <>
                                    <input disabled={true}
                                      className="disabled"
                                      type="checkbox" id={"checkboxOne" + index}
                                      value="Rainbow Dash" />
                                    <label htmlFor={"checkboxOne" + index}>
                                      {(timingOpen + index) < 13 ? (timingOpen + index) : (timingOpen + index - 12)}:00{(timingOpen + index) < 13 ? "AM" : "PM"}
                                    </label>
                                  </>
                                  :
                                  <>

                                    <input className={dateSlotArr.includes(((timingOpen + index) < 13 ? (timingOpen + index) : (timingOpen + index - 12)) + ":00" + ((timingOpen + index) < 13 ? "AM" : "PM")) ? "red" : ""}
                                      disabled={dateSlotArr.includes(((timingOpen + index) < 13 ? (timingOpen + index) : (timingOpen + index - 12)) + ":00" + ((timingOpen + index) < 13 ? "AM" : "PM")) ? true : false}
                ...
                <div className="card my-5">
                  <div className="card-header">Booking Price(&#8377;{turf.price}/hr)</div>
                  <div className="card-body">
                    Amount : {turf ? (price * turf.price).toFixed(2) : 0.00}
                  </div>
                </div>
                <button onClick={IsLoggedIn} className='btn btn-secondary'>Book Now</button>
              </div>
            </div>
            <div className="row">
      ...
      <Modal show={show} onHide={handleClose} >
        ...
        <Modal.Body>If You Want To Book this Turf Slot, Then You Have To Login First</Modal.Body>
        <Modal.Footer>
          <button onClick={GoToLogin} className="btn btn-sm btn-primary">Login</button>

          <button onClick={handleClose} className="btn btn-sm btn-secondary">Close</button>
